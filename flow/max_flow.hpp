...
public:
  ...
  // Dinic O(|E||V|^2) -> bfs O(V), dfs O(VE)
  T flow(int s, int t) {
    T flow = 0;
    while (bfs(s, t)) {
      start.assign(n, 0);
...
private:
  ...
  // 終点からdfsをすることで始点から終点に辿り着けないパスを探索しない。
  T dfs(int to, int s, T limit) {
    if (to == s) { return limit; }
    // s - ... - from - to - ... - t

    T res = 0;
    // 無駄な辺を何度も調べないために、各頂点に既に調べた辺を持たせる。各頂点でdfsを続きからできるようにするイメージ
  ...
  // 参考: FordFulkerson O(max_flow * E)
  // 貪欲法+逆辺による押し戻しにより最大流問題を解くアルゴリズム
  // std::vector<int> used;
  // int check = 1;
