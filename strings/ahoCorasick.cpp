...
// TODO: どこらへんで最長共通接尾辞を判断してるのか。。理解が曖昧なので理解する。
class AhoCorasick {
  public:
...
void JSC2019FINAL_E() {
  ...
  for(int i = 0; i < Q; ++i) {
    ...
    for(ll j = 0; j < n; ++j) {
      ...
      while(cur != -1 && !aho.hasChild(cur, T[i][j])) cur = aho.suffixLink(cur);
      // failure linkがあったらたどる、なかったら根に戻る。
      // T[i]の先頭の文字を削ることに値する。中途半端な削り方はこの処理で同時にskipされる。
      if(cur == -1) cur = 0;
      else cur = aho.nextState(cur, T[i][j]);
    }
